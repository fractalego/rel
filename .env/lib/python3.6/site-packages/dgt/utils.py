...
def create_list_of_states(metric, relations_metric, graph_list, match):
    pre_match = []
    post_match = []
    post_thresholds = []
    substitutions = []
...
def order_pre_post_matches_according_to_substitutions(pre_match, post_match, substitutions, nodes_or_relations=0):
    ...
    for i in range(len(substitutions)):
        ...
        l_vector += [['dummy', torch.zeros(post_match[i][0][1].shape[0]).to(device)] for _ in
                     range(10 - len_post_items)]
...
def create_list_of_states_for_relations(nodes_metric, metric, graph_list, match):
    pre_match = []
    post_match = []
    post_thresholds = []
    substitutions = []
...
def train_a_single_path(path, goal, metric, relation_metric, no_threshold_match, threshold_match, optimizer, epochs):
    for i in range(epochs):
        ...
        relations_pre_match, relations_post_match, relations_post_thresholds, substitutions \
            = create_list_of_states_for_relations(metric, relation_metric, graph_list, no_threshold_match)
        ...
        _, _, _, substitutions = create_list_of_states(metric, relation_metric, new_graph_list, threshold_match)
        if substitutions:
