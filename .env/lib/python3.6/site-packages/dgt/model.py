...
class DGT:
    ...
    def fit(self, epochs=20, step=5e-3, relaxation_epochs=50, relaxation_step=1e-3):
        ...
        for fact, goal in zip(self._data, self._goals):
            ...
            finished_paths, metric, relations_metric = train_all_paths(self._metric, self._relations_metric, self._k,
                                                                       end_graphs, goal, i,
                                                                       1, epochs, step)
            if finished_paths:
        ...
        for _ in range(relaxation_epochs):
            ...
            for i, path, goal in shifts_and_finished_paths:
                _, metric, relations_metric = train_all_paths(self._metric, self._relations_metric, self._k, path, goal,
            ...
            for path in paths[1]:
                for rule in path[2]:
                    if type(rule) == GraphRule:
                        self._used_rules.append(rule.predicates())
