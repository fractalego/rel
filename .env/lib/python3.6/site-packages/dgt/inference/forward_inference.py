...
from dgt.utils import get_matching_variables
    ...
    def __apply_clause_to_graph(self, rule, data):
        ...
        if not rule.gradient:
            ...
            match = Match(matching_code_container=DummyCodeContainer(),
                          node_matcher=VectorNodeMatcher(self.knowledge._metric,
                                                         self.knowledge._relations_metric, gradient=True))
            matching_variables = get_matching_variables(match, drs._g, rule.get_hypothesis()._g, self.knowledge._metric)
            #### HERE YOU HAVE TO SET THE RULE HYPOTHESIS TO BE THE ONE THAT BEST MATCHES THE DATA DRS
            ####   => CREATE A NEW get_matching_variables() that returns the graph as well
