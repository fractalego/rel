...
class GloveMetric(MetricBase):
    ...
    def __init__(self, word2vec_model, threshold=0.7):
        self._vector_map = {}
    ...
    def get_vector_index(self, text, gradient=True):
        ...
        threshold = torch.autograd.Variable(torch.Tensor(np.array([threshold])).to(device), requires_grad=gradient)
    ...
    def copy_vector_index(self, index, gradient):
        ...
        self._last_index += 1
        embedding = torch.autograd.Variable(self._vector_map[old_index], requires_grad=gradient).to(device)
        self._vector_map[self._last_index] = (embedding, threshold)
        return self._last_index
